...
- Build classes that produce objects with a `belongs_to` and `has_many`
  relationship.
...
artist instance to return the list of songs that artist has.
...
We can do a number of useful things with this collection of real song objects,
...
As it currently stands, we have to _first_ create a song and _then_ add it to a
...
_before_ an artist creates it.

Instead, let's build a method `#add_song_by_name`, that takes in an argument of
a name and genre and both creates the new song _and_ adds that song to the
artist's collection.
...
on a separate line _every time_ we want to add one to an artist––but it makes
more sense.
